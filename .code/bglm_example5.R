...
day.brm = brm(bf(BARNACLE ~ TREAT), data=day,
              family=poisson(link='log'), 
              iter = 5000, warmup = 1000,
              chains = 3, thin = 5, refresh = 0)
...
options(width=100)
prior_summary(day.brm)
options(width=80)


## ----fitModel2d, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE-------
day.brm1 = brm(bf(BARNACLE ~ TREAT), data=day,
               family=poisson(link='log'), 
...
ggpredict(day.brm1) %>% plot(add.data=TRUE)
conditional_effects(day.brm1) %>%  plot(points=TRUE)


## ----fitModel2h, results='markdown', eval=TRUE, hidden=TRUE, cache=TRUE-------
day.brm2 = brm(bf(BARNACLE ~ TREAT), data=day,
               family=poisson(link='log'), 
...
day.brm3 <- update(day.brm2,  sample_prior=TRUE, refresh=0)
...
preds <- posterior_predict(day.brm3,  nsamples=250,  summary=FALSE)
day.resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = day$BARNACLE,
                            fittedPredictedResponse = apply(preds, 2, median),
                            integerResponse = TRUE)
...
tidyMCMC(day.brm3$fit, estimate.method='median',  conf.int=TRUE,  conf.method='HPDinterval',  rhat=TRUE, ess=TRUE)
...
day.brm3 %>% get_variables()
day.draw <- day.brm3 %>% gather_draws(`.Intercept.*|.*TREAT.*`,  regex=TRUE)
day.draw
...
day.brm3 %>% plot(plotfun='mcmc_intervals') 
...
day.brm3 %>% emmeans(pairwise~TREAT, type='response')
...
day.em = emmeans(day.brm3, pairwise~TREAT, type='link')$contrasts %>%
    gather_emmeans_draws() %>%
    mutate(Fit=exp(.value))
day.em %>% head
day.em %>% group_by(contrast) %>%
...
cmat<-cbind('Alg2_Alg1'=c(-1,1,0,0),
             ...
             'Alg_NB'=c(0.5,0.5,-1,0))
# On the link scale
emmeans(day.brm3, ~TREAT, contr=list(TREAT=cmat), type='link')
# On the response scale
emmeans(day.brm3, ~TREAT, contr=list(TREAT=cmat), type='response')

day.em = emmeans(day.brm3, ~TREAT, contr=list(TREAT=cmat), type='link')$contrasts %>%
      gather_emmeans_draws() %>% mutate(Fit=exp(.value)) 
day.em %>% group_by(contrast) %>% mean_hdi()
# Probability of effect
day.em %>% group_by(contrast) %>% summarize(P=sum(Fit>1)/n())
...
newdata = emmeans(day.brm3, ~TREAT, type='response') %>%
    as.data.frame
newdata
## A quick version
g2 <- ggplot(newdata, aes(y=rate, x=TREAT)) +
